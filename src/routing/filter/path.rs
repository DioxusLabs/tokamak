...
pub trait PartBuilder: Send + Sync {
    ...
    fn build(
        ...
        sign: String,
        args: Vec<String>,
    ) -> Result<Box<dyn PathPart>, String>;
}
...
static PART_BUILDERS: Lazy<PartBuilderMap> = Lazy::new(|| {
    ...
    map.insert(
        "num".into(),
        Arc::new(Box::new(CharPartBuilder::new(is_num))),
    );
    map.insert(
        "hex".into(),
        Arc::new(Box::new(CharPartBuilder::new(is_hex))),
    );
    RwLock::new(map)
});
    ...
    fn detect<'a>(&self, state: &mut PathState) -> bool {
        ...
        if self.name.starts_with('*') {
            ...
            if let Some(cap) = cap {
                state
                    .params
                    .insert(self.name.clone(), cap.as_str().to_owned());
                state.cursor += segment.len();
                true
            } else {
                false
            ...
            if ch == '<' {
                ...
                if ch == ':' {
                    ...
                    if !is_slash {
                        ...
                        let lb = self
                            .curr()
                            .ok_or_else(|| "path ended unexcept".to_owned())?;
                        let args = if lb == '[' || lb == '(' {
                            let rb = if lb == '[' { ']' } else { ')' };
                        ...
                        let builder = builders
                            ...
                            .ok_or_else(|| {
                                format!(
                                    "PART_BUILDERS does not contains fn part with sign {}",
                                    sign
                                )
                            })?
                            .clone();
...
impl PathFilter {
        ...
        B: PartBuilder + 'static,
    {
        PART_BUILDERS
            .write()
            .unwrap()
            .insert(name, Arc::new(Box::new(builder)));
    }
