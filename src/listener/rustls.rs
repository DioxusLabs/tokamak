...
impl RustlsConfig {
    ...
    pub fn build_server_config(mut self) -> Result<ServerConfig, Error> {
        ...
        let key = {
            ...
            let mut pkcs8 =
                pkcs8_private_keys(&mut key_vec.as_slice()).map_err(|_| Error::Pkcs8ParseError)?;

            if !pkcs8.is_empty() {
                pkcs8.remove(0)
            } else {
                let mut rsa =
                    rsa_private_keys(&mut key_vec.as_slice()).map_err(|_| Error::RsaParseError)?;
    ...
    C::Item: Into<Arc<ServerConfig>>,
{
    ...
    ) -> Result<RustlsListener<C>, hyper::Error> {
        Ok(RustlsListener {
