    ...
    C::Item: Into<Identity>,
{
    ...
    ) -> Result<NativeTlsListener<C>, hyper::Error> {
...
impl NativeTlsListener<stream::Once<Ready<Identity>>> {
    ...
    ) -> NativeTlsListenerBuilder<stream::Once<Ready<Identity>>> {
        ...
        if let Some(acceptor) = this.acceptor {
            match ready!(Pin::new(this.incoming).poll_accept(cx)) {
                Some(Ok(sock)) => {
                    ...
                    let stream = NativeTlsStream::new(sock.remote_addr().into(), async move {
