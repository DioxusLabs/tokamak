...
impl Response {
    ...
    pub fn render_json<T: Serialize>(&mut self, data: &T) {
        ...
        match serde_json::to_writer(&mut cache, data) {
            ...
            Err(_) => self.set_http_error(
                InternalServerError().with_summary("error when serialize object to json"),
            ),
        }
    }
    ...
    pub fn redirect_temporary<U: AsRef<str>>(&mut self, url: U) {
        ...
        self.headers
            .insert(header::LOCATION, url.as_ref().parse().unwrap());
    }
    /// Redirect found.
    #[inline]
...
mod test {
    ...
    async fn test_others() {
        ...
        assert_eq!(
            response.take_bytes().await.unwrap().len(),
            b"response body".len()
        );
