...
impl From<Request> for http::Request {
    fn from(request: Request) -> http::Request {
        request.req
    }
}

impl<State: Default> From<http_types::Request> for Request {
    fn from(request: http_types::Request) -> Request {
        Request::new(State::default(), request, vec![])
    }
}

impl<State: Clone + Send + Sync + 'static> From<Request> for Response {
    fn from(mut request: Request) -> Response {
...
impl<'a, State> IntoIterator for &'a Request {
...
impl<'a, State> IntoIterator for &'a mut Request {
